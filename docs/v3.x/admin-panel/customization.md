...
By default, the front-development server runs on port `localhost:8000`. However, you can change this setting by updating the following configuration:
...
module.exports = ({ env }) => ({
  ...
  port: env.int('PORT', 1337),
  admin: {
    host: 'my-host', // only used along with strapi develop --watch-admin
    port: 3000, // only used along with strapi develop --watch-admin
  },
};
```
